...
def load_classifier(classifier_file):
    ...
    spec = importlib.util.spec_from_file_location("Classifier", classifier_file)
...
def evaluate(args, dataset):
    with open(args.prediction_path, "w", encoding="utf-8") as f:
        # 写入表头
        f.write("True Label\tPredicted Label\tClassification Vector\n")
        ...
        for i, (src_batch, tgt_batch, seg_batch, _) in enumerate(batch_loader(batch_size, src, tgt, seg)):
            ...
            classification_vectors = logits.cpu().numpy()
            pred = torch.argmax(nn.Softmax(dim=1)(logits), dim=1)
            gold = tgt_batch
            for j in range(pred.size()[0]):
                vector_str = str(classification_vectors[j]).replace("\n", " ")
                f.write(f"{gold[j].item()}\t{pred[j].item()}\t{vector_str}\n")
...
def main():
    ...
    parser.add_argument("--evaluate_output_path", type=str, default='fine-tuning/confusion_matrix_bilstm_v1.txt',
                        help="Evaluation result output file location.")
    parser.add_argument("--bigru_segment", type=int, default=1,
                        help="将bigru分成n段落")
    ...
    # Load classifier.
    Classifier = load_classifier(args.classifier_file)
